import { ArrowRight, BookOpen, Clock3, Flame, Heart, Search, SlidersHorizontal } from 'lucide-react'
...
export function Dashboard({ navigate, favoritesOnly = false }: { navigate: (path: string) => void; favoritesOnly?: boolean }) {
  ...
  const filtered = sampleDecks.filter(deck => deck.title.toLowerCase().includes(query.toLowerCase()) && (!favoritesOnly || favorites.includes(deck.id)))

  return <div className="page-wrap">
    <section className="welcome-row">
      <div><span className="eyebrow">QUARTA-FEIRA, 22 DE JULHO</span><h1>{favoritesOnly ? 'Seus favoritos' : <>Olá, {name}. <em>Vamos aprender?</em></>}</h1><p>{favoritesOnly ? 'Os decks que você guardou para encontrar mais rápido.' : 'Seu próximo pequeno avanço começa agora.'}</p></div>
    ...
    <section className="library-section">
      <div className="section-heading"><div><span className="eyebrow">SUA BIBLIOTECA</span><h2>{favoritesOnly ? 'Decks salvos' : 'Escolha seu próximo deck'}</h2></div><div className="search-row"><label className="search-box"><Search /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Buscar deck" /></label><button className="filter-button" aria-label="Filtrar"><SlidersHorizontal /></button></div></div>
      <div className="deck-grid">
        {filtered.map(deck => <article className="deck-card" key={deck.id}>
          <div className="deck-visual" style={{ background: deck.accent }}><span>{deck.symbol}</span><button className={favorites.includes(deck.id) ? 'favorited' : ''} onClick={() => setFavorites(old => old.includes(deck.id) ? old.filter(id => id !== deck.id) : [...old, deck.id])} aria-label="Favoritar deck"><Heart /></button></div>
