...
function loadAuthState(): { session: Session | null; expired: boolean } {
  try {
    ...
    if (!saved?.token || !saved.email || !saved.user?.id) {
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value: AuthContextValue = {
    ...
    // Atualiza os dados do usuário na sessão atual (nome/e-mail editados em Configurações)
    // sem precisar deslogar/logar de novo.
    updateProfile: (user: User) => {
      if (!session) return
      persist({ ...session, user, email: user.email })
    },
